...
for sample_ind in range(0, source_samples):
    ...
    for target in [0]:
        ...
        if current_class == 0:
            break
        adv_x , res , percent_perturb = jsma(sess , x, predictions , grads,X_test_scaled[sample_ind: (sample_ind+1)],target , theta=1, gamma =0.1, clip_min=0, clip_max =1) 
...
pred_adv = km.predict(X_adv)
normalized_mutual_info_score(y_test_l, pred_adv)
